...
<p>上图展示了 VuePress 的核心流程以及 <RouterLink to="/zh/reference/plugin-api.html">Plugin API</RouterLink> 的 Hooks ：</p>
<ul>
<li>在 <strong>init</strong> 阶段：
<ul>
<li>主题和插件会被加载。这意味着插件需要在初始化之前使用。</li>
<li>页面文件会被加载。由于我们要使用 markdown-it 来解析 Markdown 文件，因此 <RouterLink to="/zh/reference/plugin-api.html#extendsmarkdown">extendsMarkdown</RouterLink> 会在这之前调用。</li>
</ul>
</li>
...
<ul>
<li>Bundler 会被加载。由于 <RouterLink to="/zh/reference/plugin-api.html#alias">alias</RouterLink> 和 <RouterLink to="/zh/reference/plugin-api.html#define">define</RouterLink> 依赖于 Bundler 的配置，所以它们会在此处调用。</li>
</ul>
</li>
</ul>
</template>